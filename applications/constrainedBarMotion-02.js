...
Algebra(2,0,1,()=>{
  ...
  var lambda = .5,    // controls how the speed of rotation of R in 
    ...
    // first part is follows from the definition of v (velocity in body) via v'=g*v; 
    // second part says v' is v rotated 90 degrees CCW
    dState = ([g,v])=>[g*v,(v*1e12).Grade(2)];  
  ...
  var integrate = true;  // two methods for generating the animation: solve equation of motion, or geometrically
   ...
   document.body.appendChild(this.graph(
      ()=>{ 
        ...
        M = rotor>>>Mb;
        if (!paused) momcoll.collect([mcol,M]);
        var graphpt = point(
          lerp(0,400,-1,1,intenscoll.getCount()), 
          lerp(0,4,0,1,strength),
          1);
        if (!paused) intenscoll.collect(graphpt);
        ...
        return [0x0, "Constrained motion of a triangle using 2D PGA", 
